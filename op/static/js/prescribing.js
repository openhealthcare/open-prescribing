...
(function(context, namespace){
    ...
    Drug         = Models.Drug = Backbone.Model.extend({
        ...
        url: function(){
            return App.config.api_uri() + 'drug/' + this.get('bnf_code');
        },
    ...
    OPMap = Backbone.Marionette.ItemView.extend({
        ...
        initialize: function(opts){
            ...
            // CCG level data
            if(App.metadata.ccgs &&  App.metadata.ccgs.length >  0){
                this.ccgs = App.metadata.ccgs;
            ...
            if(opts.practices){
                if(App.metadata.practices && App.metadata.practices.length > 0){
                    this.practices = App.metadata.practices;
...
');
    ...
    Views.Affordance =  Backbone.Marionette.ItemView.extend({
        ...
        template: function(serialised){
            tpl = _.template('<%= name %> <img src="/static/img/throbber.gif" / >');
            return tpl(serialised)
        }
